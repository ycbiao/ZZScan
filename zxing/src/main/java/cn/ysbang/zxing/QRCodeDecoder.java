...
//        ALL_HINT_MAP.put(DecodeHintType.PURE_BARCODE, Boolean.TRUE);
    ...
    public static String syncDecodeQRCode(String picturePath) {
        ScanResult scanResult = syncDecodeQRCode(BGAQRCodeUtil.getDecodeAbleBitmap(picturePath));
     ...
     * @return 返回二维码图片里的内容 或 null
     */
    public static ScanResult syncDecodeQRCode(Bitmap bitmap) {
        Result result;
...
//            scanResult = new ScanResult(result.getText(),result.getBarcodeFormat());
//            return scanResult;
//        } catch (Exception e) {
//            e.printStackTrace();
//            if (source != null) {
...
//        }
        ...
        try {
            if (bitmap != null) {
                ...
                // 2.塞给zxing进行decode
                result = decodeYUVByZxing(bmpYUVBytes, bitmapWidth, bitmapHeight);
...
//                    qrCode = decodeYUVByZbar(bmpYUVBytes, bitmapWidth, bitmapHeight);
     ...
     * @return
     */
    private static Result decodeYUVByZxing(byte[] bmpYUVBytes, int bmpWidth, int bmpHeight) {
